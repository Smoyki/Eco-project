...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Обработчик: срабатывает при открытии materialbox
  document.addEventListener('onOpen', function(e) {
    if (e.target.classList.contains('materialboxed')) {
      mainNav.classList.add('main-nav--hidden');
    }
  }, true);

  // Обработчик: срабатывает при закрытии materialbox
  document.addEventListener('onClose', function(e) {
    if (e.target.classList.contains('materialboxed')) {
