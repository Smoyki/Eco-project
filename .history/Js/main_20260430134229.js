...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function goToSlide(slideIndex) {
    // Проверка границ: не позволяем уйти до слайда < 0 или после последнего слайда
    if (slideIndex < 0) {
      currentSlide = 0;
  ...
  // Функция обновления состояния кнопок (отключение/включение)
  function updateButtonStates() {
    prevBtn.disabled = currentSlide === 0;
    nextBtn.disabled = currentSlide === totalSlides - 1;

    // Визуальное отключение кнопок (опционально)
    if (prevBtn.disabled) {
