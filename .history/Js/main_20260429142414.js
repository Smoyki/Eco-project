...
// Делаем кнопку активной
  ...
  const cardWidth = cardsRow.scrollWidth / 2; // Ширина двух карточек
  let currentPosition = 0;

  function scrollCards(direction) {
    if (direction === 'next') {
      currentPosition -= cardWidth;
      if (Math.abs(currentPosition) >= cardsRow.scrollWidth - cardsRow.clientWidth) {
        currentPosition = 0; // Зацикливаем прокрутку
      }
    } else if (direction === 'prev') {
      currentPosition += cardWidth;
      if (currentPosition > 0) {
        // Переходим к концу для плавного зацикливания
        currentPosition = -(cardsRow.scrollWidth - cardsRow.clientWidth);
      }
    }
  ...
  cardsRow.style.transition = 'transform 0.3s ease-in-out';
});