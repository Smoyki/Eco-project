...
(function () {
  ...
  function initGlide() {
    // Проверяем, существует ли контейнер .glide
    const glideElement = document.querySelector('.glide');
    if (!glideElement) return;

    // Настройка слайдера: два видимых слайда (card_1+card_2 на старте, затем card_3+card_4)
    const glide = new Glide('.glide', {
      type: 'slider',
      perView: 2,             // показываем 2 карточки одновременно
      startAt: 0,             // начинаем с первой пары (card_1 и card_2)
      gap: 20,                // промежуток между слайдами (дополняется padding в CSS)
      bound: true,            // запрет бесконечной прокрутки
      breakpoints: {
        860: {
          perView: 1,     // на мобильных устройствах — по одной карточке
          gap: 16
        }
      },
      autoplay: false,        // только ручное управление стрелками
      animationDuration: 400
    });

    glide.mount();

    // Пересчёт слайдера при изменении размера окна для корректной адаптации
    window.addEventListener('resize', function () {
      if (glide && typeof glide.update === 'function') {
        setTimeout(function () {
          glide.update();
        }, 80);
      }
