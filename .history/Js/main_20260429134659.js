...
// Делаем кнопку активной
  ...
  // Код для кнопок переключения 
  document.addEventListener('DOMContentLoaded', function() {
  const cardsRow = document.querySelector('.cards-row');
  const prevButton = document.querySelector('.nav-arrow--prev');
  const nextButton = document.querySelector('.nav-arrow--next');
  let currentIndex = 0;
  const cardWidth = 50; // ширина карточки в процентах
  const totalCards = document.querySelectorAll('.service-card').length;
  const visibleCards = 2; // количество видимых карточек
  ...
  function scrollToCard(index) {
    ...
    // Вычисляем позицию прокрутки в пикселях
    const cardElement = document.querySelector('.service-card');
    const cardActualWidth = cardElement ? cardElement.offsetWidth + 20 : 0; // +20px — margin между карточками
    const scrollPosition = index * cardActualWidth;
  ...
  function updateButtonsState() {
    ...
    // Визуальное изменение непрозрачности для неактивных кнопок
  ...
  window.addEventListener('resize', function() {
    // При изменении размера может измениться ширина карточек — пересчитываем позицию
