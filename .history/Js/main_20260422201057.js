...
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.addEventListener('click', function(e) {
    const target = e.target.closest('.materialboxed');
    if (target) {
      // При клике на картинку скрываем меню
      mainNav.style.display = 'none';

      // Отслеживаем закрытие: Materialbox закрывает клик по overlay или ESC
      const onClose = () => {
        mainNav.style.display = ''; // Восстанавливаем исходное значение display
        document.removeEventListener('click', onClose);
        document.removeEventListener('keydown', onClose);
      };

      // Слушаем закрытие через клик по overlay
